...
export const SettingItem = ({
  ...
  const toggleAction = (
    <label htmlFor='default-toggle' className='inline-flex relative items-center cursor-pointer'>
      ...
      />
      <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-neutral-300 dark:peer-focus:ring-green-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-green-600"></div>
    </label>
  );

  const selectAction = (
    <select
      className='h-8 w-28 font-light text-sm'
      onChange={onUpdateSetting}
  ...
  return (
    <div className='flex justify-between align-baseline items-center py-2 px-1'>
      <div>
        <h1 className='text-bold text-base text-black'>{title}</h1>
